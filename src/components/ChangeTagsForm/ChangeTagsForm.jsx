    ...
    <Form>
        ...
        <label>
            ...
            <Select onChange={onChangeTagColor}>
                <SelectOption color='#ffffff' disabled selected>Choose new tag color</SelectOption>
                {colors.map(color => <SelectOption color={color} value={color}>{color}</SelectOption>)}
            </Select>
        </label>
        <Button type='submit' disabled={!Boolean(tagTitle) || !Boolean(tagColor)} title='Create Tag' onClick={onSubmit} />
